...
class PreparedDirectedSegmentTorch:
    """
    center_s -> cumulative distance along the path.
    center_heading -> heading values.
    center_curvature -> curvature values. (computed from torch.gradient and is very robust to un-even center_s)
    progress_samples -> progress values. (select the increasing parts of the lengths, ignore the flat/stopping points)
    progress_time_lookup -> time values. (select the increasing parts of the time, ignore the flat/stopping points)
    total_distance_m -> total distance along the path.
    dense_ds -> just input dense_ds
    gt_exact_speed_profile -> exact speed values. (compurted computed from a simplified version of torch.gradient on the center_s and segment_time)
    gt_longitudinal_jerk_profile -> longitudinal jerk values. (computed computed from a simplified version of torch.gradient on the gt_exact_speed_profile and segment_time)
    """
...
def cumulative_distance_torch(xy: torch.Tensor) -> torch.Tensor:
    """
    Compute the cumulative distance along a path defined by a sequence of xy coordinates.
    Args:
        xy: (N, 2) tensor of xy coordinates.
    Returns:
        distance: (N,) tensor of cumulative distances.
    """
    if xy.shape[0] == 0:
        return torch.zeros(0, dtype=xy.dtype, device=xy.device)
    deltas = torch.diff(xy, dim=0) # (N-1, 2)
    segment_lengths = torch.linalg.norm(deltas, dim=-1) # (N-1,)
    distance = torch.zeros(xy.shape[0], dtype=xy.dtype, device=xy.device) # (N,)
    distance[1:] = torch.cumsum(segment_lengths, dim=0) # (N,)
    return distance


def strictly_increasing_param_torch(param: torch.Tensor) -> torch.Tensor:
    """
    Ensure that a parameter tensor is strictly increasing.
    Args:
        param: (N,) tensor of parameters.
    Returns:
        param: (N,) tensor of strictly increasing parameters.

    Goal: Given a tensor like [3.0, 1.0, 1.0, 2.0, 5.0], make it strictly go up: [3.0, 3.000001, 3.000002, 3.000003, 5.000004]. 
    We add a small offset to each element, so that the tensor is strictly increasing.

    Why not just use cummax directly?
    Because cummax alone only gives non-decreasing (allows equal values like [1, 1, 1]). The ε ramp is what injects the strict gap between equal elements.
    """
    if param.numel() < 2:
        return param
    ...
    offset = eps * torch.arange(param.shape[0], device=param.device, dtype=param.dtype) # (N,) with values 0.0, 1.0e-6, 2.0e-6, ..., (N-1) * 1.0e-6
    base = param - offset # (N,) with values param[0] - 0.0, param[1] - 1.0e-6, param[2] - 2.0e-6, ..., param[N-1] - (N-1) * 1.0e-6
...
def interp1d_torch(x: torch.Tensor, y: torch.Tensor, xq: torch.Tensor) -> torch.Tensor:
    """
    Interpolate a 1D function defined by (x, y) at query points xq.
    Args:
        x: (N,) tensor of x values.
        y: (N,) tensor of y values.
        xq: (M,) tensor of query x values.
    Returns:
        yq: (M,) tensor of interpolated y values.

    This requires x to be strictly increasing.
    If x is not strictly increasing, use strictly_increasing_param_torch(x) to make it strictly increasing.
    """
    if x.numel() == 1:
...
def interp_heading_torch(x: torch.Tensor, heading: torch.Tensor, xq: torch.Tensor) -> torch.Tensor:
    """
    Interpolate a heading function defined by (x, heading) at query points xq.
    Args:
        x: (N,) tensor of x values.
        heading: (N,) tensor of heading values.
        xq: (M,) tensor of query x values.
    Returns:
        headingq: (M,) tensor of interpolated heading values.
...
def build_progress_time_lookup_torch(
    ...
    """
    Build a lookup table for progress and time?
    This only keeps the points where the progress is strictly increasing.
    If the result is 0.3 0.4 0.5 0.6 0.7, then all will be kept.
    if the result is 0.3 0.4 0.3 0.6 0.7, then only 0.3 0.4 0.6 0.7 will be kept.
    But then if the result is 0.6 0.7 0.3 0.4 0.5, then only 0.6 0.7 0.4 0.5 will be kept. the result is not strictly increasing. 
    ? wait for confirm
    Args:
        progress: (N,) tensor of progress values.
        time: (N,) tensor of time values.
    Returns:
        progress_lookup: (M,) tensor of progress values.
        time_lookup: (M,) tensor of time values.
    """
...
def make_prepared_directed_segment_torch(
    ...
    """
    Prepare the dataclass structure.
    center_s -> cumulative distance along the path.
    center_heading -> heading values.
    center_curvature -> curvature values. (computed from torch.gradient and is very robust to un-even center_s)
    progress_samples -> progress values. (select the increasing parts of the lengths, ignore the flat/stopping points)
...
def heading_from_positions_torch(
    ...
    """
    Convert xy coordinates to heading. 
    All existing function call contain fallback heading.

    For segment that is too short, it will fallback to the fallback heading.
    Args:
        xy: (N, 2) tensor of xy coordinates.
        fallback_heading: (N,) tensor of fallback heading values.
    Returns:
        heading: (N,) tensor of heading values.
    ...
    if num_points == 1:
        if fallback_heading is not None:
            return fallback_heading[:1] # ? shape is correct?
...
def speed_from_progress_torch(progress: torch.Tensor, time: torch.Tensor) -> torch.Tensor:
    """
    Convert progress to speed.
    Args:
        progress: (N,) tensor of progress values.  (x)
        time: (N,) tensor of time values.  (t)
    Returns:
        speed: (N,) tensor of speed values. (dx/dt)

    This is a simple version for torch.gradient if the time is essentailly uniform.
    """
    if time.shape[0] < 3:
...
def acceleration_from_speed_torch(speed: torch.Tensor, time: torch.Tensor) -> torch.Tensor:
    ...
    Args:
        ...
        time: (N,) tensor of time values. (t)
    Returns:
        acceleration: (N,) tensor of acceleration values. (d^2x/dt^2)
    """
...
def jerk_from_acceleration_torch(acceleration: torch.Tensor, time: torch.Tensor) -> torch.Tensor:
    """
    Convert acceleration to jerk.
    Args:
        acceleration: (N,) tensor of acceleration values. (d^2x/dt^2)
        time: (N,) tensor of time values. (t)
    Returns:
        jerk: (N,) tensor of jerk values. (d^3x/dt^3)
    """
...
def curvature_from_xy_torch(xy: torch.Tensor, param: torch.Tensor) -> torch.Tensor:
    """
    In practise, param -> is the cumulative distance along the path.
    Args:
        xy: (N, 2) tensor of xy coordinates.
        param: (N,) tensor of cumulative distance along the path.
    Returns:
...
"""
The Core Idea: Quintic Hermite Interpolation

Both functions are built on the same two quintic Hermite basis polynomials evaluated at normalized arc-length

u = s / s_merge is the normalized arc-length.

The primal gives you where the vehicle should be laterally; the derivative gives you which direction it should be heading.

在自动驾驶中，车辆需要从当前车道平滑地横向偏移到目标位置（比如换道、避障）。
本质上是：用 Quintic Hermite 的前两个基函数，描述一个"从偏移归零"的过程
"""

"""
Comment:
These augmentation has an assumption:
- p(0) = lateral_offset  --> correct
- p'(0) = d(lateral_offset)/ds = tan(heading_offset) -> correct
- p''(0) = d(tan heading_offset)/ds = 0 --> This is not correct, it ignore the current steering or the acceleration.

-------

So in practice, we will replan in a high frequency
t = 0 we have a bias, and we correctly predict the future trajectory assuming currently we have not provide lateral steering.
t = 0.1, we already apply some lateral steering, and we predict the future trajectory again,
   - This time, because of the model trained on this augmentation, 
         it will still predict the future trajectory as if we have not provided lateral steering.
      - This is not correct and the new trajectory will be "smoother" and the heading adjustment will be postponed.
"""
...
def lateral_offset_profile_torch(
    ...
    s              : (N,)      — N 个弧长采样点
    s_merge        : ()        — 标量，归并距离
    lateral_offset : ()        — 标量，起点横向偏移
    heading_offset : ()        — 标量，起点朝向偏差
...
def sample_centerline_torch(
    ...
    """
    center_s    : (N,)      — N 个弧长采样点
    center_xy   : (N, 2)    — N 个 xy 坐标
    center_heading : (N,)    — N 个 heading 值
    query_s     : (M,)      — M 个弧长采样点

    output      : (M, 2)    — M 个 xy 坐标
    output      : (M,)      — M 个 heading 值
    """
...
def build_offset_path_torch(
    ...
    """
    Samping based on the dense_s, and apply the lateral offset and heading offset briding augmentation.
    center_xy   : (N, 2)    — N 个 xy 坐标
    center_heading : (N,)    — N 个 heading 值
    center_s    : (N,)      — N 个弧长采样点
    s_merge     : ()        — 标量，归并距离
    ...
    output      : (M, 2)    — M 个 xy 坐标  M = num_dense + 1 = ceil(s_merge / dense_ds) + 1
    output      : (M,)      — M 个 cumulative distance along the path
    output      : (M,)      — M 个 heading 值
    """
...
def merge_path_length_torch(
    ...
    curvature = interp1d_torch(center_s, center_curvature, dense_s) # (M,) get the interpolated curvature values

    # get the briding augmentated offset.
    offset = lateral_offset_profile_torch(
        dense_s,
        torch.clamp(s_merge, min=1.0e-6),
        lateral_offset,
    ...
    # get the briding augmented lateral offset derivative. (lateral restoration speed)
    ...
    ) ## The magnitute of the dr/ds vector.
    ds = torch.diff(dense_s) # (M-1,) the distance between each sample point.
    return torch.sum(0.5 * (integrand[:-1] + integrand[1:]) * ds) # get the true path length.
...
def solve_merge_centerline_s_torch(
    ...
    center_curvature: torch.Tensor,
    center_s: torch.Tensor,
    distance_budget: torch.Tensor, # how many meters of travel you are allowed during the bridge window 
    lateral_offset: torch.Tensor,
    heading_offset: torch.Tensor,
    dense_ds: float,
    tol_m: float = 1.0e-3,
    ...
    """
    Binary search to find the merge point.
    """
    device = center_xy.device
    dtype = center_xy.dtype
    ...
    while lower > dense_ds * 1.0e-3:
        # Compute the new all path length
    ...
    if upper_length < distance_budget:
        ## If the path length is less than the distance budget, it means the bridge window/target path is too short.
...
def build_full_augmented_path_torch(
    center_xy: torch.Tensor, ## based centerline
    center_heading: torch.Tensor,
    center_s: torch.Tensor,
    merge_xy: torch.Tensor, ## augmented points
    merge_sigma: torch.Tensor,
    merge_heading: torch.Tensor,
    s_merge: torch.Tensor, ## The distance to the merge point
    total_distance_m: torch.Tensor, ## full distance of the original centerline
    dense_ds: float,
) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    """
    Build the full augmented path with the augmented points (merge_xy) and the continuing part (interpolate/sample from the original centerline)
    """
    device = center_xy.device
    dtype = center_xy.dtype
    continuation_s = torch.arange(
        ...
        device=device,
        dtype=dtype,
    ) ## the continuing part of the path after the merge point.
...
def sample_dense_path_torch(
    ...
    """
    Sample the dense path based on the cumulative distance.
    path_sigma   : (M,)      — M 个 cumulative distance along the path
    path_xy      : (M, 2)    — M 个 xy 坐标
    path_heading : (M,)      — M 个 heading 值
    query_sigma  : (N,)      — N 个 cumulative distance along the path

    output      : (N, 2)    — N 个 xy 坐标
    output      : (N,)      — N 个 heading 值
    """
...
def augment_segment_prepared_torch(
    ...
    """
    Augment the segment based on the prepared segment. 
    - The merging distance is searched based on the build offset path length.
    - The new augmented path is the joint of the brdiging augmented path and the continuing part of the original centerline.
    """
...
def augment_segment_torch(
    ...
    """
    - Create the dataclass structure of the prepared segment,
    - Call the augment_segment_prepared_torch to search for distance budget and create the briding.
    """
...
class StatePerturbation:
    ...
    def _sample_lateral_offsets(self, batch_size: int) -> torch.Tensor:
        """
        Sample lateral offsets from a uniform distribution between -0.75 and +0.75.
        Tensor size: [batch_size, ]
        """
        random_tensor = torch.rand(batch_size, device=self._device)
        return self._low[:, 1] + (self._high[:, 1] - self._low[:, 1]) * random_tensor # -0.75 ~ +0.75

    def _sample_heading_offsets(self, batch_size: int) -> torch.Tensor:
        """
        Sample heading offsets from a uniform distribution between -0.1745 and +0.1745.
        Tensor size: [batch_size, ]
        """
        random_tensor = torch.rand(batch_size, device=self._device)
        return self._low[:, 2] + (self._high[:, 2] - self._low[:, 2]) * random_tensor # -0.1745 ~ +0.1745
    ...
    def augment(self, inputs, ego_future):
        ...
        lateral_offsets = self._sample_lateral_offsets(batch_size) # [batch_size, ]
        heading_offsets = self._sample_heading_offsets(batch_size) # [batch_size, ]
        valid_speed = torch.abs(ego_current_state[:, 4]) >= 2.0 # [batch_size, ]
        valid_offset = (torch.abs(lateral_offsets) > 1.0e-3) | (torch.abs(heading_offsets) > 1.0e-3) # [batch_size, ]

        ## Only augment if the speed is greater than 2.0 m/s and the offset is greater than 1.0e-3
        aug_flag = (
            (torch.rand(batch_size, device=self._device) < self._augment_prob) # [batch_size, ]
            & valid_speed # [batch_size, ]
        ...
        for batch_index in torch.nonzero(aug_flag, as_tuple=False).flatten():
            ...
            best_result = None
            for _ in range(8): # gradually reduce the noise offset by half if the sample is not feasible
                try:
                    best_result = self._search_feasible_sample(
                        ...
                        lateral_offset=lateral_offset,
                        heading_offset=heading_offset,
                        initial_past_connect_time_s=self._past_bridge_sec, # 1.0 s
                        initial_future_recover_time_s=self._future_bridge_sec, # 1.5 s
                    )
                    break
                except RuntimeError: # if the sample is not feasible, reduce the noise offset by half
