...
def test_straight_line_searches_merge_and_rejoins_gt() -> None:
    ...
    assert torch.allclose(
        aug_future[0, merge_idx:, :2], orig_future[0, merge_idx:, :2], atol=1e-4
    )
...
def test_peak_can_sit_in_history() -> None:
    ...
    t_hist = -3.0
    assert info["history_merge_s"] == pytest.approx(
        max(t_hist, float(info["t_b_s"]) - 2.0)
    )
...
def test_dwell_history_does_not_flip_heading() -> None:
    ...
    orig_h = torch.atan2(
        inputs["ego_agent_past"][0, :, 3], inputs["ego_agent_past"][0, :, 2]
    )
    aug = _make_tau_augmentor()
...
def test_kappa_rate_rejects_short_violent_merge() -> None:
    ...
    limits = KinematicLimits()
    assert not _bump_window_ok(
        t, base_vel, base_acc, base_jerk, bias, -0.8, -0.2, 0.5, limits
    )
    assert _bump_window_ok(
        t, base_vel, base_acc, base_jerk, bias, -2.0, -0.2, 2.5, limits
    )
